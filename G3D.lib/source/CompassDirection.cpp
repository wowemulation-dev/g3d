...
static const std::string NAME_TABLE[33 * 2] = {
"North", "N",
"North by east", "NbE",
"North-northeast", "NNE",
...
const std::string& CompassDirection::nearestCompassPointName() const {
...
const std::string& CompassDirection::nearestCompassPointAbbreviation() const {
    // Change names every 12.5 degrees, offset slightly
    const int i = iFloor(wrap(m_angleDegrees + (360 / 64.0f), 360) / 32);
    debugAssertM(i >= 0 && i < 32, "Index out of bounds");
